...
- **触发方式**：push 一个 `vX.Y.Z` tag（例如 `v1.0.1`）
...
**重要规则**：tag 必须等于 `v<package.json version>`，例如 `version=1.0.1` → tag=`v1.0.1`
...
仓库 → `Releases` → 对应 `v1.0.1` release 中，通常会包含：
...
1) 从 Releases 下载旧版本安装包（例如 `v1.0.0`）
2) 安装并启动一次，确认能正常打开
